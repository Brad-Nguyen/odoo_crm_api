    ...
    <nav class="bg-slate-900 text-white p-4 flex justify-between items-center shadow-xl">
        ...
        <div class="flex gap-4">
            <form action="/api/import" method="post" enctype="multipart/form-data" class="bg-white/5 p-1 rounded border border-white/10 flex items-center">
                ...
                <button type="submit" class="bg-blue-600 px-2 py-1 rounded text-[10px] font-bold">IMPORT</button>
            </form>
...
async def home():
    ...
    <div class="flex justify-between items-center mb-10">
        <h1 class="text-3xl font-black uppercase tracking-tighter text-slate-800">Pipeline <span class="text-blue-600">PRO</span></h1>
...
def card(i):
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 relative group">
        ...
        <div class="flex justify-between items-center border-t pt-3">
            <span class="text-[9px] bg-slate-100 px-3 py-1 rounded-full font-black uppercase tracking-widest text-slate-500">{i.get('status', 'Mới')}</span>
...
async def edit_view(id: str):
    ...
    <div class="max-w-md mx-auto bg-white p-10 rounded-3xl shadow-2xl mt-12 border border-slate-100">
        <h2 class="text-2xl font-black mb-8 text-center uppercase">Chỉnh sửa</h2>
        <form action="/api/update/{id}" method="post" class="flex flex-col gap-6 text-left">
            ...
            <a href="/" class="text-center text-gray-400 font-bold text-sm mt-4">HUỶ</a>
        </form></div>"""
    return layout(content)
...
async def import_csv(file: UploadFile = File(...)):
    # FIX: Đảm bảo đọc file an toàn và insert đúng database
    try:
        ...
        return HTMLResponse(f"Lỗi Import: {str(e)}")